...
class cmb_nse_library(simcache.sims_tpmap.library):
    ...
    def get_dat_pmap(self, det):
        return ( hp.read_map( dmc.get_fname_iqumap(self.year, det, self.forered), hdu=1, field=1 ) +
                 hp.read_map( dmc.get_fname_iqumap(self.year, det, self.forered), hdu=1, field=2 ) * 1.j ) * 1.e3
    ...
    def simulate_nse(self, det):
        ...
        if self.ntype == 'nobs':
            ...
            q_nse = rtnobsinv * dmc.sigma0[(self.year, self.forered, 'P')][det] * 1e3 * np.random.standard_normal( npix ) * np.sqrt(2.)
            u_nse = rtnobsinv * dmc.sigma0[(self.year, self.forered, 'P')][det] * 1e3 * np.random.standard_normal( npix ) * np.sqrt(2.)
            
        elif self.ntype == 'nobs_qucov':
            tfname =  dmc.get_fname_iqumap(self.year, det, False) #NOTE: Nobs counts taken from non-FG reduced maps.
            npix = 12*self.nside**2

            # temperature
            t_nse = (dmc.sigma0[(self.year, self.forered, 'T')][det] * 1e3 /
                     np.sqrt( hp.read_map( tfname, hdu=1, field=3 ) )) * np.random.standard_normal( npix )

            # polarization
            cov_inv_qq = hp.read_map( tfname, hdu=2, field=1 ) / (dmc.sigma0[(self.year, self.forered, 'P')][det] * 1.e3)**2
            cov_inv_qu = hp.read_map( tfname, hdu=2, field=2 ) / (dmc.sigma0[(self.year, self.forered, 'P')][det] * 1.e3)**2
            cov_inv_uu = hp.read_map( tfname, hdu=2, field=3 ) / (dmc.sigma0[(self.year, self.forered, 'P')][det] * 1.e3)**2
